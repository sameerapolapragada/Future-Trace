...
create index plan_features_plan_id_idx on public.plan_features (plan_id, sort_order);
...
create table public.industry_risks (
  id uuid primary key default gen_random_uuid(),
...
create index industry_risks_industry_id_idx on public.industry_risks (industry_id, sort_order);

create table public.job_exposure_groups (
  id uuid primary key default gen_random_uuid(),
  slug text not null unique,
  title text not null,
...
create table public.job_exposure_examples (
  id uuid primary key default gen_random_uuid(),
...
create index job_exposure_examples_group_id_idx on public.job_exposure_examples (group_id, sort_order);
...
create index milestones_status_sort_idx on public.milestones (status, sort_order);
create index milestones_period_idx on public.milestones (period_start_year, period_end_year);

create table public.milestone_display (
...
create table public.milestone_sections (
  id uuid primary key default gen_random_uuid(),
  milestone_id uuid not null references public.milestones (id) on delete cascade,
...
create index milestone_sections_milestone_id_idx on public.milestone_sections (milestone_id, sort_order);
...
create index milestone_sources_milestone_id_idx on public.milestone_sources (milestone_id, sort_order);

create table public.milestone_industry_tags (
  milestone_id uuid not null references public.milestones (id) on delete cascade,
  industry_name text not null,
  primary key (milestone_id, industry_name)
);

create table public.milestone_job_tags (
  milestone_id uuid not null references public.milestones (id) on delete cascade,
...
-- ---------------------------------------------------------------------------
create table public.score_jobs (
  id uuid primary key default gen_random_uuid(),
  slug text not null unique,
  title text not null,
...
create index exposure_scores_user_created_idx on public.exposure_scores (user_id, created_at desc);
create index exposure_scores_job_idx on public.exposure_scores (score_job_id);
...
create index content_feed_items_feed_published_idx on public.content_feed_items (feed_type, published_at desc);
...
create table public.search_topics (
  id uuid primary key default gen_random_uuid(),
  label text not null,
  milestone_id uuid references public.milestones (id) on delete set null,
...
create index search_topics_type_sort_idx on public.search_topics (topic_type, sort_order);
...
create index milestones_fts_idx on public.milestones
  using gin (
    to_tsvector(
      'english',
